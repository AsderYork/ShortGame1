...
namespace Voxels
{
	...
	{
		return char(std::min(std::max((float)std::numeric_limits<char>::min(), (float)ceil(abs(value) /*+ 0.5f*/)) * (value > 0 ? 1 : -1), (float)std::numeric_limits<char>::max()));
	}
					...
					for (unsigned z = 0u; z < BLOCK_EXTENTS; ++z)
					{
						for (unsigned y = 0u; y < BLOCK_EXTENTS; ++y)
						{
							for (unsigned x = 0u; x < BLOCK_EXTENTS; ++x)
							{
								...
								char height = std::clamp((int)slice - (int)heightmap[row * w + column], -127, 127);
	...
	void VoxelGrid::IdentifyTouchedBlocks(const btVector3& position, const btVector3& extents, std::vector<TouchedBlock> & touchedBlocks)
	{
		const btVector3 changeMin = position - extents;
		const btVector3 changeMax = position + extents;
		...
		btVector3 blockExtents(blockExtDiv2, blockExtDiv2, blockExtDiv2);
		...
		for (unsigned blZ = 0u; blZ < blocksY; ++blZ)
			for (unsigned blY = 0u; blY < blocksY; ++blY)
				for (unsigned blX = 0u; blX < blocksX; ++blX)
				{
					const btVector3 blockCenter(blX * BLOCK_EXTENTS + blockExtDiv2,
						blY * BLOCK_EXTENTS + blockExtDiv2,
						blZ * BLOCK_EXTENTS + blockExtDiv2);

					const btVector3 blockMin = blockCenter - blockExtents;
					const btVector3 blockMax = blockCenter + blockExtents;

					if (IntersectBoxes(changeMin, changeMax, blockMin, blockMax))
					{
						const btVector3 blockBase(float(blX * BLOCK_EXTENTS),
							float(blY * BLOCK_EXTENTS),
							float(blZ * BLOCK_EXTENTS));
						BlockExtents blockExt = std::make_pair(blockBase, blockBase + btVector3((const float)BLOCK_EXTENTS, (const float)BLOCK_EXTENTS, (const float)BLOCK_EXTENTS));
	...
	void VoxelGrid::CalculateTouchedBlockSection(
		...
		btVector3& end)
	{
		const btVector3 extDiv2(extents.x() / 2, extents.y() / 2, extents.z() / 2);
		const btVector3 initialChangePos = position - extDiv2;

		start = btVector3(std::clamp(initialChangePos.x(), blockExt.first.x(), blockExt.second.x()) - blockExt.first.x(),
		...
		end = btVector3(std::clamp(initialChangePos.x() + extents.x(), blockExt.first.x(), blockExt.second.x()) - blockExt.first.x(),
	...
	std::pair<btVector3, btVector3> VoxelGrid::InjectSurface(
		const btVector3& position,
		const btVector3& extents,
		VoxelSurface * surface,
		InjectionType type)
	{
		...
		std::for_each(touchedBlocks.begin(), touchedBlocks.end(),
			[&](TouchedBlock & touched)
			{
				...
				btVector3 surfaceCoordStart = blockExt.first + blockStart - position;
				btVector3 surfaceCoordEnd = blockExt.first + blockEnd - position;
		...
		const btVector3 initialChangePos = position - (extents / 2.0f);

		// DX- style coords!
		const auto globalStart = btVector3(std::max(0.f, initialChangePos.x()),
			std::max(0.f, initialChangePos.z()),
			std::max(0.f, initialChangePos.y()));
		const auto globalEnd = btVector3(std::min(float(m_Width), globalStart.x() + extents.x()),
	...
	std::pair<btVector3, btVector3> VoxelGrid::InjectMaterial(
		const btVector3& position,
		const btVector3& extents,
		MaterialId material,
		bool addSubtractBlend)
	{
		...
		const btVector3 extDiv2(extents / 2.0f);
		const btVector3 extDivCoeff = extDiv2 * 0.75f;
			...
			[&](TouchedBlock & touched)
			{
				...
				for (float z = blockStart.z(); z < blockEnd.z(); ++z)
					for (float y = blockStart.y(); y < blockEnd.y(); ++y)
						for (float x = blockStart.x(); x < blockEnd.x(); ++x)
						{
							btVector3 currentPosition = btVector3(x + blockExt.first.x(),
								y + blockExt.first.y(),
								z + blockExt.first.z());

							const auto voxelId = VoxelIdInBlock(unsigned(x), unsigned(y), unsigned(z));
							const auto dist = (currentPosition - position).length() / extDivCoeff.x();
	...
	void VoxelGrid::GetBlockData(const btVector3 & blockCoords, char* output) const
	{
	...
	void VoxelGrid::GetMaterialBlockData(const btVector3 & blockCoords, unsigned char* materialOutput, unsigned char* blendOutput) const
	{
		const auto id = CalculateInternalBlockId(blockCoords);
	...
	bool VoxelGrid::IsBlockEmpty(const btVector3 & blockCoords) const
	{
		const auto id = CalculateInternalBlockId(blockCoords);
	...
	void VoxelGrid::ModifyBlockDistanceData(const btVector3 & coords, const char* distances)
	{
	...
	void VoxelGrid::ModifyBlockMaterialData(const btVector3 & coords, const MaterialId * materials, const BlendFactor * blends)
	{
		const auto id = CalculateInternalBlockId(coords);
		Block& block = m_Blocks[id];
		m_MemoryForBlocks -= block.MaterialData.size();
	...
	float3pair toFloat3Pair(const std::pair<btVector3, btVector3> & resultInternal)
	{
		float3pair result;
	...
	float3pair Grid::InjectSurface(const float3 & position,
		...
		InjectionType type)
	{
		const auto internalPos = btVector3(position.x, position.y, position.z);
		const auto internalExt = btVector3(extents.x, extents.y, extents.z);
		auto resultInternal =
	...
	{
		const auto internalCoords = btVector3(coords.x, coords.y, coords.z);
