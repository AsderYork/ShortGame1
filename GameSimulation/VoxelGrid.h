...
namespace Voxels
{
	...
	public:
		...
		std::pair<btVector3, btVector3> InjectSurface(
			const btVector3& position,
			const btVector3& extents,
			VoxelSurface* surface,
			InjectionType type);

		std::pair<btVector3, btVector3> InjectMaterial(
			const btVector3& position,
			const btVector3& extents,
			MaterialId material,
			bool addSubtractBlend);

		void GetBlockData(const btVector3& blockCoords, char* output) const;
		void GetMaterialBlockData(const btVector3& blockCoords, unsigned char* materialOutput, unsigned char* blendOutput) const;
		bool IsBlockEmpty(const btVector3& blockCoords) const;

		inline unsigned CalculateInternalBlockId(const btVector3& blockCoords) const;

		inline btVector3 GetBlocksCount() const;

		void ModifyBlockDistanceData(const btVector3& coords, const char* distances);
		void ModifyBlockMaterialData(const btVector3& coords, const MaterialId* materials, const BlendFactor* blends);
	...
	private:
		...
		typedef std::pair<btVector3, btVector3> BlockExtents;
		typedef std::pair<unsigned, BlockExtents> TouchedBlock;
		void IdentifyTouchedBlocks(const btVector3& position, const btVector3& extents, std::vector<TouchedBlock>& touchedBlocks);

		static void CalculateTouchedBlockSection(const btVector3& position,
			const btVector3& extents,
	...
	unsigned VoxelGrid::CalculateInternalBlockId(const btVector3& blockCoords) const
	{
	...
	btVector3 VoxelGrid::GetBlocksCount() const
	{
		return btVector3(m_Width / BLOCK_EXTENTS,
